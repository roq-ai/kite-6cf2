...
import { useRoqClient, useTutorFindManyWithCount } from 'lib/roq';
import { convertQueryToPrismaUtil } from 'lib/utils';

type ColumnType = ColumnDef<TutorInterface, unknown>;
...
export function TutorListPage(props: TutorListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const { data, error, isLoading, mutate } = useTutorFindManyWithCount({
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await roqClient.tutor.delete({ where: { id } });
      await mutate();
    } catch (error) {
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/tutors/view/${record.id}`} passHref legacyBehavior>
                <Button
