...
import { useRoqClient, useTutorFindFirst } from 'lib/roq';
...
function TutorEditPage() {
  ...
  const { data, error, isLoading, mutate } = useTutorFindFirst(queryParams, {}, { disabled: !id });
  ...
  const handleSubmit = async (values: TutorInterface, { resetForm }: FormikHelpers<any>) => {
    ...
    try {
      ...
      const updated = await roqClient.tutor.update({
        data: values as RoqTypes.tutor,
        where: {
          id,
        },
      });
    ...
    >
      <Box rounded="md">
        ...
        <FormWrapper wrapperProps={{ mb: 3 }}>
          <ImagePicker ref={imageUploaderRef} onChange={handleImageChange} entity="tutor" entityId={id} />
        </FormWrapper>
